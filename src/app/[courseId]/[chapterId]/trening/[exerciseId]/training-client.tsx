...
import { detectExerciseType, type ExerciseType } from "@/lib/training/exercise-type";
...
export function TrainingClient({
  ...
  const exerciseType = useMemo(() => {
    return detectExerciseType(exercise, courseId);
  }, [exercise, courseId]);
  ...
  useEffect(() => {
    if (exerciseType !== 'unknown' && (hasNumericAnswers || hasExpressionAnswers) && problems.length === 0 && !showResults) {
  ...
  // Check if exercise has no trainable content (or no generator for this exercise type)
  if (exerciseType === 'unknown' || (!hasNumericAnswers && !hasExpressionAnswers)) {
    return (
      <div className="flex min-h-screen flex-col">
        ...
        <main id="main-content" className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">Denne oppgaven har ikke treningsmodus ennå. Treningsoppgaver lages foreløpig bare for 1T-bokas kapitler om potenser, algebra, brøk, faktorisering, rasjonale uttrykk og likninger.</p>
