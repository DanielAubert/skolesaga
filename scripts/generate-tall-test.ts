/**
 * Generer test-lyd for tall-uttale i ElevenLabs.
 * Tester alle tall som brukeren ønsker å verifisere.
 */
...
async function main() {
  console.log('=== Generer test-lyd for tall-uttale ===\n');

  const text = `Her er en test av hvordan tall uttales.
...
hundre. hundre og en. hundre og tjuefire.

syv hundre. syv tusen.`;
  ...
  const chunkPath = resolve(OUTPUT_DIR, '_temp_tall_chunk.mp3');
  const cleanPath = resolve(OUTPUT_DIR, '_temp_tall_clean.mp3');
  const masterPath = resolve(MASTER_DIR, 'tall-test.mp3');
