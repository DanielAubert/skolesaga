#!/usr/bin/env node
/**
 * Omset quiz (Test deg sjølv) frå bokmål til nynorsk — extract/apply, INGEN betalt API.
 * Kjelde: src/lib/data/quiz/_all.json  (dump frå dump-quiz.mjs)
 * Sidecar: src/lib/data/quiz/nn/<chapterId>.json = { questions:[...], _meta:{tm,...} }
 *
 * Omsettbart per spørsmål: question, kvar option (BEHALD rekkjefølgja — fyrste er rett svar),
 * og explanation. Ingenting anna.
 *
 * Bruk:
 *   node scripts/translate-quiz-nynorsk.mjs --extract <id...> --out /tmp/q-todo.json
 *   node scripts/translate-quiz-nynorsk.mjs --apply /tmp/q-done.json
 */
import fs from 'fs';
...
const QUIZ_DIR = path.join(ROOT, 'src', 'lib', 'data', 'quiz');
const NN_DIR = path.join(QUIZ_DIR, 'nn');

const sha = (s) => crypto.createHash('sha256').update(s).digest('hex').slice(0, 16);

let ALL = null;
function getAll() {
  if (!ALL) ALL = JSON.parse(fs.readFileSync(path.join(QUIZ_DIR, '_all.json'), 'utf-8'));
  return ALL;
}
...
// Returner liste av {get,set} for alle omsettbare strenger i ein quiz-array.
function collectRefs(questions) {
  const refs = [];
  for (const q of questions) {
    if (typeof q.question === 'string' && worthTranslating(q.question))
      refs.push({ get: () => q.question, set: (v) => { q.question = v; } });
    if (Array.isArray(q.options)) {
      for (let i = 0; i < q.options.length; i++) {
        if (worthTranslating(q.options[i]))
          refs.push({ get: () => q.options[i], set: (v) => { q.options[i] = v; } });
      }
    }
    if (typeof q.explanation === 'string' && worthTranslating(q.explanation))
      refs.push({ get: () => q.explanation, set: (v) => { q.explanation = v; } });
  }
  return refs;
}
...
function srcFor(id) {
  ...
  return { raw, srcHash: sha(raw), questions: JSON.parse(raw) };
}
...
function extractOne(id, { force = false } = {}) {
  ...
  const refs = collectRefs(c.questions);
  const seen = new Set();
  const need = [];
...
if (opts.apply) {
  ...
  const outPath = opts.out || path.join(ROOT, 'tmp-q-todo.json');
